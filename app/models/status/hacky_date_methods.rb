class Status
  ...
  def set_finished_at=(new_time)
    raise "Bad input format" unless new_time =~ /^(\d{2})[:](\d{2})$/
    #day = $1.to_i < created_at.hour ? 1 : 0 # add a day to the created_at if it's earlier
    
    t = Time.parse("#{new_time} %+.2u00" % (Time.zone.utc_offset/3600))
    day = t.hour > created_at.hour ? 1 : 0
    self.finished_at = Time.mktime(created_at.year, created_at.month, created_at.day, t.hour, t.min) + day.day
  end
