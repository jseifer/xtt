class Spec::Example::AwesomeExample < Spec::Example::Example
  ...
  #
  #   it.validates_uniqueness_of Foo, :name
  #
  def validates_uniqueness_of(model, *attributes)
    attributes.each do |attr|
      desc, imp = assert_validates_uniqueness_of(model, attr)
      if @defined_description then create_sub_example desc, &imp
      else @defined_description, @implementation = desc, imp end
    end
  end
...
protected
  ...
  def assert_validates_presence_of(model, attribute)
    ["validates presence of #{attribute}", lambda do
  ...
  def assert_validates_uniqueness_of(model, attribute)
    ["validates uniqueness of #{attribute}", lambda do
      @attributes.each do |key, value|
        @record.send("#{key}=", value)
      end
      @record.save
      violated "Couldn't save with the default attributes: #{@record.errors.full_messages.to_sentence}" if @record.new_record?
