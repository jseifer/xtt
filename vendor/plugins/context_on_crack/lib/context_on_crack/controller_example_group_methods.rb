module ContextOnCrack
  ...
  class ControllerAccessProxy < ControllerProxy
    class << self
      attr_accessor :proxy_source
    end
    self.proxy_source = ControllerAccessGroup

    attr_accessor :proxy_source

    def initialize(test_case, controller)
      @proxy_source = self.class.proxy_source
      super test_case, controller, "access"
    end

    def as(*users, &block)
      users.each do |user|
        @proxy_source.new(self, user).instance_eval(&block)
      end
    end
  end
      ...
      it "renders #{format}" do
        ...
        acting do |response|
          ...
          if block
            ...
            response.body.should == instance_eval(&block)
          end
        end
      end
    end
